...
// Backend returns lowercase — keep everything lowercase here
const WORKFLOW = ['pending', 'processing', 'shipped', 'completed']

const STATUS_META = {
  pending:    { color: '#F59E0B', bg: '#FFFBEB', dot: '#F59E0B', next: 'processing', label: 'Pending'    },
  processing: { color: '#6C47FF', bg: '#EDEAFF', dot: '#6C47FF', next: 'shipped',    label: 'Processing' },
  shipped:    { color: '#9B6DFF', bg: '#F3EEFF', dot: '#9B6DFF', next: 'completed',  label: 'Shipped'    },
  completed:  { color: '#10B981', bg: '#ECFDF5', dot: '#10B981', next: null,         label: 'Completed'  },
}
...
function WorkflowStepper({ status }) {
  const key = status?.toLowerCase()
  const cur = WORKFLOW.indexOf(key)
  return (
...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setOrders(toArray(oRes.data))
      setSummary(sRes.data)
      setNotifications(nRes.data?.notifications || [])
    } catch {
      setError('Failed to load dashboard. Is Django running?')
    } finally {
      setLoading(false)
    }
  }, [user])
  ...
  return (
    <>
      ...
      {error && (
        <div style={{ background: '#FEE2E2', color: '#EF4444', padding: '12px 16px', borderRadius: 12, marginBottom: 20, fontWeight: 600, fontSize: 13 }}>
          ⚠️ {error}
        </div>
      )}
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '80px 0' }}>
          ...
          <p style={{ color: '#C4B8E8', fontWeight: 600 }}>Loading dashboard...</p>
        </div>
      ) : (
        <>
          {/* Stats */}
          ...
          <div style={{
            ...
            border: '1.5px solid #F0EBFF', marginBottom: 16,
              ...
              return (
                <div key={s} style={{ display: 'flex', alignItems: 'center' }}>
                  <div onClick={() => setFilter(s)} style={{
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, color: filter === s ? '#fff' : m.color, letterSpacing: '0.06em', textTransform: 'uppercase', marginTop: 2 }}>{m.label}</span>
                  </div>
                  {i < WORKFLOW.length - 1 && (
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px 14px', borderBottom: '1.5px solid #F7F4FF', flexWrap: 'wrap', gap: 10 }}>
              <h2 style={{ fontSize: 16, fontWeight: 800, color: '#2D1F6E' }}>
                All Orders
                {filter !== 'All' && <span style={{ marginLeft: 8, fontSize: 13, color: '#9B6DFF', fontWeight: 600 }}>— {STATUS_META[filter]?.label}</span>}
              </h2>
            ...
            <div style={{
              ...
              {notifications.slice(0, 5).map((n, i) => (
                <div key={n.id ?? i} style={{
                  ...
                  <div style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0, marginTop: 4, background: '#9B6DFF' }} />
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 13, color: '#2D1F6E', fontWeight: 600, lineHeight: 1.4 }}>{n.message}</p>
